...
function CreateVending()
            ...
            options = {
                {
                    ...
                    action = function()
                        OpenVending(Config.VendingLabel, Config.VendingItem)
                    end,
                }
            },
            distance = 1.5,
    ...
    for k, v in pairs(Config.WaterObjects) do
        exports['qb-target']:AddTargetModel(v, {
            options = {
                {
                    icon = 'fa-solid fa-bag-shopping',
                    label = "Water Cooler",
                    action = function()
                        OpenVending(Config.WaterLabel, Config.WaterItems)
                    end,
                }
            },
            distance = 1.5,
    ...
    for k, v in pairs(Config.CoffeObjects) do
        exports['qb-target']:AddTargetModel(v, {
            options = {
                {
                    icon = 'fa-solid fa-bag-shopping',
                    ...
                    action = function()
                        OpenVending(Config.CoffeLabel, Config.CoffeItems)
                    end,
                }
            },
            distance = 1.5,
...
function OpenVending(label, items)
    local ShopItems = {}
    ShopItems.label = label
    ShopItems.items = items
    ShopItems.slots = #items
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "Vendingshop_"..math.random(1, 99), ShopItems)
end
...
RegisterNUICallback("GiveItem0", function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('lj-inventory:server:GetClosePlayersName', function(players)
        ...
        for k, v in pairs(players) do
            for g, c in pairs(Config.MaskList) do
                local mask = GetPedDrawableVariation(GetPlayerPed(GetPlayerFromServerId(v.serverId)), c)
                if mask > 0 then
                    v.name = "Anonim"
                    isMask = true
                    break
