...
local function RemoveItem(source, item, amount, slot)
	...
	if item == "bodycam" then
		TriggerClientEvent('gct-bodycam:close', Player.PlayerData.source)
	end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if itemDataTest.amount >= toAmount then
						...
						TriggerClientEvent('qb-inventory:client:closeinv', src)
						exports["JD_logs"]:discord("``["..src.."]`` ID' li oyuncu ``"..itemDataTest.name.."`` adlı eşyayı **DUPE** yapmaya çalıştı!", src, 0, "RED", "Dupe")
					end
				end
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			...
			if Player.Functions.RemoveMoney("cash", price, "unkown-itemshop-bought-item") then
				...
				if shopType == "police" then
					if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
						TriggerEvent("qb-log:server:CreateLog", "policearmory", "Armory Item Bought", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.."** bought a ***" .. itemInfo["label"] .. "*** with ``"..itemData.info.serie.."`` *Serial Number* for ``$"..price.."``")
					else
						TriggerEvent("qb-log:server:CreateLog", "policearmory", "Armory Item Bought", "green", "**"..Player.PlayerData.charinfo.firstname .. " ".. Player.PlayerData.charinfo.lastname.."** bought a ***" .. itemInfo["label"] .. "*** for ``$"..price.."``")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				end
